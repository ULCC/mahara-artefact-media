function FileBrowser(idprefix, config, globalconfig) {
   ...
   // this.folderid = folderid;
    ...
    this.upload_submit = function (e) {
        ...
        // clear out the values used for title and description
        //$(self.id + '_episodetitle').value = '';
        self.form[self.id+'_episodetitle'].value = '';
        $(self.id + '_episodedescription').value = '';
...
//    /**
//     * Empties the user supplied values from a form element
//     */
//    this.clearcontents = function(elementname) {
//        var element = 'self.form.'+elementname;
//        element.value = '';
//
//    }
    ...
    this.upload_success = function (data) {
        ...
        self.quotaUpdate(data.quotaused, data.quota);
        ...
        // uncheck the copyright agreement
        var checkbox = $(self.id+'_notice')
...
//    };
    ...
    this.quotaUpdate = function(quotaused, quota) {
        
        if (! $('mediaquota_percentage') ) {
            logWarning('Filebrowser.quotaUpdate(', quotaused, quota, ') called but no id="quota_percentage" on page');
            return;
        }
...
//            }
            ...
            var ref = $('mediaquota_bar_100') || $('mediaquota_bar');

            if (percentage < 100) {
                $('mediaquota_fill').style.display = 'block';
                if (ref.id != 'mediaquota_bar') {
                    swapDOM(ref, P({'id': 'mediaquota_bar'}, SPAN({'id': 'mediaquota_percentage'})));
                }
            }
            else {
                $('mediaquota_fill').style.display = 'none';
                if (ref.id != 'mediaquota_bar_100') {
                    swapDOM(ref, P({'id': 'mediaquota_bar_100'}, SPAN({'id': 'mediaquota_percentage'})));
                }
            }

            $('mediaquota_used').innerHTML = data.quotaused;
            $('mediaquota_total').innerHTML = data.quota;
            $('mediaquota_percentage').innerHTML = percentage + '%';
            $('mediaquota_fill').style.width = (percentage * 2) + 'px';
        }
